...
normal_binomial_model <- glm(form
                             , data = game_series
                             , family = binomial())

beta_binomial_model <- gamlss::gamlss(form
               , data = game_series
               , link = BNB())
mu_coefs <- beta_binomial_model$mu.coefficients
mu_coefs
beta_binomial_model$sigma.coefficients
...
fitted_distributions <- predict_data %>%
  mutate(fitted = predict(beta_binomial_model, newdata = predict_data %>% select(-id, -n_wins, -n_losses))
         , fitted_lm = predict(normal_binomial_model, predict_data %>% select(-id), type = 'response')) %>%
  mutate(sigma = exp(beta_binomial_model$sigma.coefficients)
         , alpha = fitted/sigma
         , beta = (1-fitted)/sigma
         , posterior_probability = 1- pnbinom(q = 3 - n_losses, size = 7 - games_played, prob = 1 - fitted, lower.tail = FALSE) ) 
