...
game7_series <- po_dat_wide %>%
  filter(series_length == 5) %>% select_if(~sum(!is.na(.)) > 0)
...
predict_data <- tribble(~id, ~win_game_1, ~win_game_2, ~p_win_game_1
                  ,'WL',TRUE, FALSE, .5
                  ,'LW', FALSE, TRUE, .5
                  ,'WW',TRUE, TRUE, .5)

fitted_distributions <- predict_data %>%
  mutate(fitted = predict(game_2_model, newdata = predict_data %>% select(-id))
         , fitted_lm = predict(lm_model, predict_data))%>%
  mutate(sigma = exp(game_2_model$sigma.coefficients)
         , alpha = fitted/sigma
         , beta = (1-fitted)/sigma
         , posterior_probability = pbinom(q = 5, size = 7, prob = fitted, lower.tail = FALSE) ) 
